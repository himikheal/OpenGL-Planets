...
int main()
{
	...
	Camera camera(width, height, glm::vec3(100.0f, 50.0f, 100.0f), earthPos);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		day += inc;
		float earthAngle = glm::radians(get_earth_rotate_angle_around_sun(day));
		float moonAngle = glm::radians(get_moon_rotate_angle_around_earth(day));
		earthPos.x = (24 * cos(earthAngle));
		earthPos.z = (-24 * sin(earthAngle));
		moonPos.x = (earthPos.x + 12 * cos(moonAngle));
		moonPos.z = (earthPos.z + (-12 * sin(moonAngle)));
		camera.changeOrientation(earthPos);
